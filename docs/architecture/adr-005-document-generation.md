...
A revision is a **draft** or **issued**. A draft can be edited, regenerated, and deleted. Issuing locks the revision, stores the file's hash with who issued it and when, and cannot be undone; a correction is a new draft revision, and issuing it marks the earlier one superseded, still downloadable. (Amended by [ADR 007](adr-007-leads-and-service-contracts.md): the new draft starts from the issued revision's file, edits included, and keeps its data snapshot. Regenerating stays beside it for starting over from the template with today's data. As first built, regenerating and uploading were the only ways to a new draft, so correcting an edited document meant redoing every edit.) Review, client approval, and signature states arrive with electronic signatures. Owners and specialists can all generate, edit, and issue.
...
The trial ran on 19 September 2026 ([write-up](../document-editor-trial.md)): the browser editor, by then `@docx-editor.dev/react` 2.21, opened, edited and saved 20 of the 23 documents without losing text or structure, and the page counts after LibreOffice matched. The three that failed do so on constructs the templates can avoid: a floating image, and cells merged down a table longer than a page. It is the editor the app embeds; the two servers were not tried.
