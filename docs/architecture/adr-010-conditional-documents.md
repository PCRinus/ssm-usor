...
A document that exists is never hidden or deleted because the fact changed: a 1.5 generated at 10 employees stays, draft or issued, when the client falls to 9, and only stops being required. Generating that 1.5 again still needs a representative, and cover 1.0 keeps listing it.
